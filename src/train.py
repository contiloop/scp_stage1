...
class GradNormTrainer(SFTTrainer):
    ...
    Features:
      - LLRD (layer-wise learning rate decay): 후반 layer에 더 낮은 lr 적용
      - Spike skip: gradient 이상 감지 시 step 건너뛰기
    """
    ...
    def create_optimizer(self):
        ...
        if self.llrd_decay < 1.0:
            ...
            defaults = {k: v for k, v in self.optimizer.param_groups[0].items()
                        if k != "params"}
